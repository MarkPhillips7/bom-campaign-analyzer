...
// copies changed jpeg files to the output directory
gulp.task('build-jpeg', function () {
  return gulp.src(paths.jpeg)
    .pipe(changed(paths.output, {extension: '.jpeg'}))
    .pipe(gulp.dest(paths.output));
});
...
gulp.task('build', function(callback) {
  return runSequence(
    'clean',
    ['build-system', 'build-html', 'build-css', 'build-jpeg', 'build-png'],
    callback
  );
});
